                ...
                <div className="px-5">
                    <div id="booking_invoice" className="px-4 border border-secondary">
                        <header className="clearfix">
                            ...
                            <div id="project">
                                <div><span>Name</span> {booking?.user?.name}</div>
                                <div><span>EMAIL</span> {booking?.user?.email}</div>
                                <div><span>DATE</span>{new Date(booking?.createdAt).toLocaleString('en-US')}</div>
                                <div><span>Status</span>{booking?.paymentInfo?.status?.toUpperCase()}</div>
                            </div>
                        </header>
                        <main>
                            <table className="mt-5">
                                ...
                                <tr>
                                    ...
                                    <td className="unit">{new Date(booking?.checkInDate).toLocaleString('en-US')}</td>
                                    <td className="qty">{new Date(booking?.checkOutDate).toLocaleString('en-US')}</td>
