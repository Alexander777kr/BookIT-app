...
import {
    ...
    useLazyCheckBookingAvailabilityQuery, useLazyStripeCheckoutQuery,
...
const BookingDatePicker = ({room}: Props) => {
    ...
    const {data: {bookedDates: dates} = {}} = useGetBookedDatesQuery({id: room?._id});
    ...
    return (
        <div className="booking-card shadow p-4">
            ...
            <p className="mt-5 mb-3">
                Pick Check In & CheckOut Date
            </p>
            <DatePicker className="w-100" selected={checkInDate} onChange={onChange} startDate={checkInDate}
                        endDate={checkOutDate} minDate={new Date()} excludeDates={excludeDates} selectsRange inline/>
            {isAvailable === true && (
            ...
            {isAvailable && isAuthenticated && (
                <button className="btn py-3 form-btn w-100" onClick={bookRoom} disabled={isLoading}>Pay -
                    ${room?.pricePerNight * daysOfStay}</button>
            )}
